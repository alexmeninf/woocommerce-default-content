...
{ ?>
  <script>
    ...
    /**
     * Mudar imagens do footer
     * (Func Exclusiva para o wp-starter-theme)
     */
    function imagesAppearance() {
      let baseLogo = 'https://assets.comet.com.br/assets/default/';

      if (isDark()) {
        document.querySelectorAll('.js-dev-footer img')[0].setAttribute('src', baseLogo + 'logo-inova-white.png');
        document.querySelectorAll('.js-dev-footer img')[1].setAttribute('src', baseLogo + 'logo-bluelizard-white.png');
      } else {
        document.querySelectorAll('.js-dev-footer img')[0].setAttribute('src', baseLogo + 'logo-inova-dark.png');
        document.querySelectorAll('.js-dev-footer img')[1].setAttribute('src', baseLogo + 'logo-bluelizard-default.png');
      }
    }
  </script>
<?php
...
add_action('wp_footer', 'footer_color_scheme', 30);
...
function callback_template_scheme()
{
  return '<form 
    id="modeAppearance" 
    aria-label="' . __("Selecione a aparência do site", "wcstartertheme") .'" 
    role="radiogroup" 
    tabindex="0" 
    class="color-scheme-toggle">
    <label>
      <input name="mode" type="radio" value="light">
      <div class="text">' . __("Clara", "wcstartertheme") .'</div>
    </label>
    <label>
      <input name="mode" type="radio" value="dark">
      <div class="text">' . __("Escura", "wcstartertheme") .'</div>
    </label>
    <label>
...
/**
 * Exibir shortcode no footer
 */
function add_shortcode_color_scheme() {
  echo '<div class="mb-3 mt-2 mt-lg-0">'. do_shortcode('[color_scheme_toggle]') .'</div>';
}

add_action('footer_before_developer', 'add_shortcode_color_scheme');