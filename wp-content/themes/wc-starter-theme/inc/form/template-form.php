 ...
 */
$api = get_field('api_url') != '' ? get_field('api_url') : THEMEROOT . '/inc/form/senderPHPMail/send.php';
...
?>
  ...
  <form id="form-theme-<?php the_ID(); ?>" action="javascript:void(0);" method="POST" class="d-flex flex-wrap <?php echo $class ?>">
    ...
    <button type="submit" class="btn-theme btn-medium">
      <span><?php echo get_field('button_text') ? get_field('button_text') : __('Enviar', 'startertheme'); ?></span>
    </button>
  </form>
  ...
  <script>
    ...
    $(form<?php the_ID(); ?>).on('submit', function(e) {
          ...
          $input_type  = get_sub_field('input_type'); ?>
            ...
            if (<?= $id ?>.trim() == "") {
              Swal.fire({
                type: 'warning',
                title: 'Oops...',
                html: '<?php printf(__("O campo <b>%s</b> é obrigatório.", 'startertheme'), $name) ?>'
              });
            } else
      ...
      $.ajax({
        ...
        beforeSend: () => {
          $(btnForm).html('<?= __("Enviando...", 'startertheme') ?>');
        }
        ...
        if (obj.success) {
          Swal.fire({
            title: '<?= __("Enviado!", 'startertheme') ?>',
            text: `${obj.message}`,
            type: 'success',
            confirmButtonText: '<?= __("Fechar", 'startertheme') ?>'
          });
        } else {
          Swal.fire({
            title: '<?= __( "Algo deu errado!", 'startertheme') ?>',
            text: `${obj.message}`,
            type: 'error',
        ...
        $(btnForm).html('<?= __("Enviar novamente", 'startertheme') ?>');
      });
