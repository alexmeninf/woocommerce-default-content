 ...
 * 
 */
function post_type_theme_form()
{
  $labels = array(
    'name'                  => _x('Formulários', 'Post Type General Name', 'startertheme'),
    'singular_name'         => _x('Formulário', 'Post Type Singular Name', 'startertheme'),
    'menu_name'             => __('Formulários', 'startertheme'),
    'all_items'             => __('Todos os formulários', 'startertheme'),
    'add_new_item'          => __('Adicionar novo formulário', 'startertheme'),
    'new_item'              => __('Novo formulário', 'startertheme'),
    'edit_item'             => __('Editar formulário', 'startertheme'),
    'update_item'           => __('Atualizar formulário', 'startertheme'),
    'search_items'          => __('Buscar formulário', 'startertheme'),
  );

  $args = array(
    'label'                 => __('Formulário', 'startertheme'),
    'description'           => __('Adicione um novo formulário de envio.', 'startertheme'),
    'labels'                => $labels,
  ...
  register_post_type('theme_form', $args);
}

add_action('init', 'post_type_theme_form', 0);
 ...
 * 
 * */
add_filter('manage_theme_form_posts_columns', 'set_custom_edit_theme_form_columns');
add_action('manage_theme_form_posts_custom_column', 'custom_theme_form_column', 10, 2);

function set_custom_edit_theme_form_columns($columns)
{
  $columns['shortcode'] = __('Shortcode', 'startertheme');

  return $columns;
}

function custom_theme_form_column($column, $post_id)
{
  switch ($column) {
    case 'shortcode':
