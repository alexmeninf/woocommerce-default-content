...
function theme_setup_lang() {
  load_theme_textdomain( 'wcstartertheme', get_template_directory() . '/languages' );
}
...
register_nav_menus(
  array(
    'primary' => esc_html__( 'Primary menu', 'wcstartertheme' ),
  )
);
...
if( function_exists('acf_add_options_page') ) {
    ...
    array(
		'page_title'  => __('Opções do site', 'wcstartertheme'),
		'menu_title'  => __('Opções do site', 'wcstartertheme'),
		'menu_slug'   => 'opcoes',
...
function the_title_page() {
  ...
  if (is_404()) {
    echo __('Página não encontrada', 'wcstartertheme');
    ...
    echo __('Registros de ', 'wcstartertheme') . $date;
    ...
    echo __('Registros de ', 'wcstartertheme') . get_the_time('Y');

  } elseif (is_author()) {
    echo __('Registros do autor', 'wcstartertheme');

  } elseif (isset($_GET['p']) && !empty($_GET['p'])) {
    echo __('Registros do blog', 'wcstartertheme');

  } elseif (is_search()) {
    echo __('Resultados da pesquisa', 'wcstartertheme');

  } else {
    if (class_exists('WooCommerce')) {
      if (is_shop()) {
        echo __('Os melhores produtos para você', 'wcstartertheme');
      } else {
        the_title();
      }
    } else {
...
function support_comments_facebook($url = '') {
  ...
  <?php if (strstr($_SERVER['HTTP_USER_AGENT'], 'iPhone') || strstr($_SERVER['HTTP_USER_AGENT'], 'iPad') || strstr($_SERVER['HTTP_USER_AGENT'], 'Mac')) : ?>
    <p class="ms-2 face-link">
      ...
      <?php _e('Não consegue comentar?', 'wcstartertheme') ?> 
      <?php _e('<a href="https://facebook.com/home.php" target="_blank" rel="noreferrer noopener" title="Conecte ao facebook" class="text-decoration-none">Conecte à sua conta do Facebook</a> em outra página e volte.', 'wcstartertheme') ?>
    </p>
  <?php endif; ?>
  ...
  if ($pages_count > 0) : ?>
    ...
    <nav aria-label="Page navigation">
      <ul class="pagination justify-content-center">
        ...
        echo '<a class="page-link" aria-label="Previous" title="' . __('Página anterior', 'wcstartertheme') . '" ' . $disable_link . ' href="' . $url . '&pg=1"><span>&laquo;</span></a>';
        echo '</li>';
        ...
        $disable_link = ($current_page == $pages_count) ? 'disabled' : 'title="' . __('Próxima página', 'wcstartertheme') . '"';
 ...
 * @return mixed
 */
function start_theme_preload() {

  if (THEME_ENABLE_PRELOAD === true) : ?>

    <div class="preload">
      <img src="<?= THEMEROOT ?>/assets/img/icons/loading.svg" alt="<?php _e('Carregando...', 'wcstartertheme') ?>" height="35" width="35">
    </div>
...
add_action('wp_body_open', 'start_theme_preload', 10);
  ...
  if (!is_front_page()) {
    ...
    if (is_archive() && !is_tax() && !is_category() && !is_tag()) {
      ...
      if (is_day()) {
        ...
        $archive_title = __('Registros de ', 'wcstartertheme') . $format_date;
    
      } elseif (is_month()) {
        $archive_title = __('Registros de ', 'wcstartertheme') . get_the_time('F, Y');
    
      } elseif (is_year()) {
        $archive_title = __('Registros de ', 'wcstartertheme') . get_the_time('Y');
      }    
      ...
      echo '<li class="'.$li_class.' item-current item-current-' . get_query_var('paged') . '"><span class="bread-current bread-current-' . get_query_var('paged') . '" title="' . __('Página', 'wcstartertheme') . ' ' . get_query_var('paged') . '">' . __('Página', 'wcstartertheme') . ' ' . get_query_var('paged') . '</span></li>';
    
    } else if (is_search()) {
      // Página Search
      echo '<li class="'.$li_class.' item-current item-current-' . get_search_query() . '"><span class="bread-current bread-current-' . get_search_query() . '" title="'.__('Resultado da pesquisa por', 'wcstartertheme').': ' . get_search_query() . '">'.__('Resultado da pesquisa por', 'wcstartertheme').': ' . get_search_query() . '</span></li>';
    
    } elseif (is_404()) {
      // Pagina 404
      echo '<li class="'.$li_class.'">' . __('Página não encontrada', 'wcstartertheme') . '</li>';
    }
...
function published_modified_date() {
  ...
  $output = '<time itemprop="datePublished" putdate datetime="'.get_the_date($utf_format).'" class="entry-date"><span>'. __('Publicado em', 'wcstartertheme') .'</span> ' . get_the_date( $date_format ) . '</time>';

	if ( $updated > ( $date + 86400 ) ) {
    $output .= '<span class="mx-2 fw-bold">·</span>';
    $output .= '<time itemprop="dateModified" datetime="'.get_the_modified_date($utf_format).'" class="entry-date-modified"><span>'. __('Atualizado em', 'wcstartertheme') .'</span> ' . get_the_modified_date( $date_format ) . '</time>';
  }
