...
class WPPB_Pagination{
	...
	function links(){
		...
		if (($this->pages) > 1){
			...
			if ($this->page != 1) {
				...
				$plinks[] = '&nbsp;<a href="' . $this->create_page_link($this->page - 1) . '" class="page-link pageLink_previous">'.$this->prev.'</a>&nbsp;';
			}
			...
			if ($this->page < $this->pages) {
				$slinks[] = '&nbsp;<a href="' . $this->create_page_link($this->page + 1) . '" class="page-link pageLink_next">'.$this->next.'</a>&nbsp;';
				if ($this->showFirstAndLast) {
	...
	function create_page_link( $counter ){
        ...
        return esc_attr( add_query_arg( $args, get_permalink() ) );
