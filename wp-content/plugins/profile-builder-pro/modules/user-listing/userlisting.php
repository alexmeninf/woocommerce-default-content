...
function wppb_userlisting_users_loop($value, $name, $children, $extra_values)
{
    if ($name == 'users') {
        ...
        if (!empty($userlisting_args[0])) {
                        ...
                        if (isset($_GET['ul_filter_' . $faceted_setting['facet-meta']])) {
                            if ($faceted_setting['facet-type'] == 'range') {
                                ...
                                if (!empty($values)) {
                                    ...
                                    }/* handle roles facet differently */ else if ($wpdb->get_blog_prefix() . 'capabilities' == $faceted_setting['facet-meta']) {
                                        $args['meta_query'][0][$faceted_setting['facet-meta']] = array('relation' => 'OR');
                                        foreach ($values as $key => $val) {
                                            $args['meta_query'][0][$faceted_setting['facet-meta']][] = array(
                                                'key' => $faceted_setting['facet-meta'],
                    ...
                    foreach ($user_meta_keys as $user_meta_key) {
                        if (!in_array($user_meta_key, $wppb_exclude_search_fields)) {
                            $args['meta_query'][2][] = array(
                                'key' => $user_meta_key,
                                'value' => apply_filters('wppb_ul_search_all_meta_value', stripslashes($search_for), $user_meta_key, $wppb_manage_fields),
            ...
            if (strpos($all_user_listing_template, '{{{faceted_menus}}}') !== false) {
                $thisPageOnly = array_unique($wp_user_search->get_results(), SORT_REGULAR); //array_unique was introduced since we introduced the compare_key arg for repeaters ..and the query can return the same user multiple times
            } else {
...
function wppb_ul_faceted_search($faceted_filter_options, $meta_values, $wppb_manage_fields)
{
    ...
    $filter  = '<div class="wppb-facet-search-wrap"><label><input type="text" value="' . $current_value . '" class="wppb-facet-search" data-filter-behaviour="' . esc_attr($faceted_filter_options['facet-behaviour']) . '" data-current-page="' . esc_attr(get_query_var('page')) . '" data-meta-name="' . esc_attr($faceted_filter_options['facet-meta']) . '">';
...
function wppb_get_all_values_for_user_meta($meta_key, $wppb_manage_fields)
{
    ...
    if (!empty($meta_key)) {
        ...
        if (!empty($all_queried_user_ids_string)) {
            ...
            }/* we need to handle the role facet differently */ else if ($meta_key == $wpdb->get_blog_prefix() . 'capabilities') {
                if (!empty($results)) {
                    $new_keys = array();
                    foreach ($results as $key => $value) {
...
function wppb_userlisting_extra_search_all_fields($value, $name, $children, $extra_info)
{
    ...
    return '
		<form method="post" action="' . esc_url(add_query_arg(array('page' => 1, 'setSortingCriteria' => $setSortingCriteria, 'setSortingOrder' => $setSortingOrder), wppb_remove_query_arg('page', wppb_curpageurl()))) . '" class="wppb-search-users wppb-user-forms">
...
add_filter('wppb_ul_search_all_meta_value', 'wppb_ul_change_country_name_to_code_in_search', 10, 3);
