...
function wppb_generate_modules_default_values(){
	...
	if ( $wppb_module_settings_description == 'not_found' ){
		$wppb_module_settings_description = 	array(	'wppb_userListing' => 'User-Listing',
													    ...
													    'wppb_emailCustomizer' => 'Email Customizer',
		...
		<form method="post" action="options.php#add-ons">
			...
			<table class="widefat column-1">
				...
				<tbody>
					...
					<tr>
						<td><?php _e( 'Email Customizer', 'profile-builder' );?></td>
						<?php
						...
						?>
						<td>
							<input id="ec_s" type="radio" name="wppb_module_settings[wppb_emailCustomizer]" value="show" <?php if ( $wppb_email_customizer_activate == 'show') echo 'checked';?> /><label for="ec_s"><?php _e( 'Active', 'profile-builder' );?></label>
							<input id="ec_h" type="radio" name="wppb_module_settings[wppb_emailCustomizer]" value="hide" <?php if ( $wppb_email_customizer_activate == 'hide') echo 'checked';?> /><label for="ec_h"><?php _e( 'Inactive', 'profile-builder' );?></label>
						</td>
					</tr>
					<tr class="alternate">
...
/*
we eleiminated the independent option for admin email customizer so we need to tie it to the user email customizer option for turning off,
