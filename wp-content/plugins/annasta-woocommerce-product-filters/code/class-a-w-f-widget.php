...
if( ! class_exists( 'A_W_F_widget' ) ) {
  class A_W_F_widget extends WP_Widget {
    ...
    public function __construct() {
      ...
      parent::__construct( 'awf_widget', __( 'annasta Filters', 'annasta-filters' ), $settings );
    ...
    public function widget( $args, $instance ) {
        ...
        /* Admin widget previews */
        
        $preset_name = __( 'annasta Filters', 'annasta-filters' );
        $presets = $this->get_presets();

        if( isset( $instance['preset_id'] ) && isset( $presets[strval( $instance['preset_id'] )] ) ) {
          $preset_name .= ': ' . $presets[strval( $instance['preset_id'] )];
        }
				echo
          '<h3 style="padding:8px;text-align:center;font-size:15px;font-weight:600;">',
    ...
    public function form( $instance ) {
      ...
      <label for="<?php echo esc_attr( $this->get_field_id( 'preset_id' ) ); ?>" class="awf-widget-label"><?php esc_html_e( 'annasta Filters Preset', 'annasta-filters' ); ?></label> 
