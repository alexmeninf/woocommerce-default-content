<?php
    ...
    /*
    Plugin Name: Profile Builder - Multi-Step Forms Add-On
    Plugin URI: https://www.cozmoslabs.com/wordpress-profile-builder/
    Description: Extends the functionality of Profile Builder by adding possibility of having multi-page registration and edit-profile forms
    Version: 1.1.1
...
/**
 * Function to run on plugin activation and check/delete break points attached to deleted fields
 *
 * @since   v.1.0.2
 */
function wppb_msf_check_break_points_on_activation() {
...
register_activation_hook( __FILE__, 'wppb_msf_check_break_points_on_activation' );
...
add_action( 'admin_enqueue_scripts', 'wppb_msf_scripts' );
 ...
 * @since   v.1.0.0
 */
function wppb_msf_frontend_assets() {
...
add_action( 'wp_footer', 'wppb_msf_frontend_assets' );
 ...
 * @since   v.1.0.0
 */
function wppb_msf_add_meta_boxes() {
    add_meta_box( 'wppb-msf-side', __( 'Multi-Step Forms', 'profile-builder' ), 'wppb_msf_meta_boxes_content', array( 'wppb-epf-cpt', 'wppb-rf-cpt', 'profile-builder_page_manage-fields' ), 'side', 'low' );
}
add_action( 'add_meta_boxes', 'wppb_msf_add_meta_boxes', 11 );
 ...
 * @since   v.1.0.0
 */
function wppb_msf_meta_boxes_content( $post ) {
    $screen = get_current_screen();
    ...
    if( $screen->id == 'profile-builder_page_manage-fields' ) {
        ...
        if( isset( $_POST['wppb_msf_save_options'] ) && wp_verify_nonce( $_POST['wppb_msf_save_options'], 'wppb-msf-options-verify' ) ) {
            ...
            if( isset( $_POST['msf-tab-title'] ) ) {
                foreach( $_POST['msf-tab-title'] as $key => $value ) {
                    if( ! empty( $value ) ) {
            ...
            echo '<div class="notice notice-success is-dismissible"><p>'. __( 'Multi-Step Forms options updated.', 'profile-builder' ) .'</p></div>';
        }

        ?>
        <form id="wppb-msf-options" name="wppb-msf-options" method="POST" action="">
            <p><strong><?php _e( 'Enable on:', 'profile-builder' ); ?></strong></p>
            <p><label><input type="checkbox" id="pb-default-register" name="pb-default-register" value="yes" <?php echo $wppb_msf_options != 'not_found' && $wppb_msf_options['pb-default-register'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'PB Default Register Form', 'profile-builder' ); ?></label></p>
            <p><label><input type="checkbox" id="pb-default-edit-profile" name="pb-default-edit-profile" value="yes" <?php echo $wppb_msf_options != 'not_found' && $wppb_msf_options['pb-default-edit-profile'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'PB Default Edit Profile Form', 'profile-builder' ); ?></label></p>
            <p class="description"><?php _e( 'To enable it on Multiple Registration and Edit-Profile Forms you must add Break Points in each form page.', 'profile-builder' ); ?></p>
            <p><strong><?php _e( 'Pagination and Tabs:', 'profile-builder' ); ?></strong></p>
            <p><label><input type="checkbox" id="msf-pagination" name="msf-pagination" value="yes" <?php echo $wppb_msf_options != 'not_found' && isset( $wppb_msf_options['msf-pagination'] ) && $wppb_msf_options['msf-pagination'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'Enable Pagination', 'profile-builder' ); ?></label></p>
            <p><label><input type="checkbox" id="msf-tabs" name="msf-tabs" value="yes" <?php echo $wppb_msf_options != 'not_found' && isset( $wppb_msf_options['msf-tabs'] ) && $wppb_msf_options['msf-tabs'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'Enable Tabs', 'profile-builder' ); ?> <span class="wppb-msf-edit-tabs-title" style="display: none" >- <a id="wppb-msf-edit-tabs-title" href="#"><?php _e( 'Edit Tabs Title', 'profile-builder' ); ?></a></span></label></p>
            <div class="wppb-msf-tabs-title-container" style="display: none;"></div>
            <div id="major-publishing-actions"><input name="save" type="submit" class="button button-primary button-large" id="wppb_msf_submit" value="<?php _e( 'Update Multi-Step', 'profile-builder' ); ?>"></div>
        ...
        if( get_post_status( $post->ID ) !== 'publish' ) {
            $wppb_msf_check_post_status = '<p class="description wppb-msf-post-publish-error">'. __( 'Publish the form before adding Break Points!', 'profile-builder' ) .'</p>';
        }

        ?>
        <?php echo $wppb_msf_check_post_status; ?>
        <p class="description"><?php _e( 'To enable MSF you must add Break Points.', 'profile-builder' ); ?></p>
        <p><strong><?php _e( 'Pagination and Tabs:', 'profile-builder' ); ?></strong></p>
        <p><label><input type="checkbox" id="msf-pagination" name="msf-pagination" value="yes" <?php echo $wppb_msf_post_options != 'not_found' && $wppb_msf_post_options['msf-pagination'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'Enable Pagination', 'profile-builder' ); ?></label></p>
        <p><label><input type="checkbox" id="msf-tabs" name="msf-tabs" value="yes" <?php echo $wppb_msf_post_options != 'not_found' && $wppb_msf_post_options['msf-tabs'] == 'yes' ? 'checked' : ''; ?>> <?php _e( 'Enable Tabs', 'profile-builder' ); ?> <span class="wppb-msf-edit-tabs-title" style="display: none" >- <a id="wppb-msf-edit-tabs-title" href="#"><?php _e( 'Edit Tabs Title', 'profile-builder' ); ?></a></span></label></p>
        <div class="wppb-msf-tabs-title-container" style="display: none;"></div>
        <?php
 ...
 * @since   v.1.0.0
 */
function wppb_msf_save_metabox_content( $post_id ) {
    ...
    if( isset( $_POST['wppb_msf_save_post_options'] ) && wp_verify_nonce( $_POST['wppb_msf_save_post_options'], 'wppb-msf-post-'. $post_id .'-options-verify' ) ) {
...
add_action( 'save_post', 'wppb_msf_save_metabox_content' );
 ...
 * @return  string
 */
function wppb_msf_output_before_first_form_field( $output, $form_id, $form_type, $form_fields, $called_from ) {
    if( ! is_null( $form_id ) ) {
...
add_filter( 'wppb_output_before_first_form_field', 'wppb_msf_output_before_first_form_field', 10, 5 );
 ...
 * @return  string
 */
function wppb_msf_output_after_last_form_field( $output, $form_id, $form_type, $called_from ) {
    if( ! is_null( $form_id ) ) {
        $break_points = get_post_meta( $form_id, 'wppb_msf_break_points', true );
        $break_points = is_array( $break_points ) ? $break_points : 'not_found';
    } else {
...
add_filter( 'wppb_output_after_last_form_field', 'wppb_msf_output_after_last_form_field', 10, 4 );
 ...
 */
function wppb_msf_output_after_form_field( $field_output, $field, $form_id, $form_type, $called_from ) {
    if( ! is_null( $form_id ) ) {
        $break_points = get_post_meta( $form_id, 'wppb_msf_break_points', true );
        $break_points = is_array( $break_points ) ? $break_points : 'not_found';
    } else {
...
add_filter( 'wppb_output_after_form_field', 'wppb_msf_output_after_form_field', 10, 5 );
 ...
 * @return  string
 */
function wppb_msf_hide_send_credentials_checkbox( $output, $form_type, $form_id ) {
    if( ! is_null( $form_id ) ) {
        $break_points = get_post_meta( $form_id, 'wppb_msf_break_points', true );
    } else {
        $break_points = get_option( 'wppb_msf_break_points', 'not_found' );
...
add_filter( 'wppb_before_send_credentials_checkbox', 'wppb_msf_hide_send_credentials_checkbox', 10, 3 );
 ...
 * @return  string
 */
function wppb_msf_hide_form_submit( $output, $form_type, $form_id ) {
    if( ! is_null( $form_id ) ) {
        $break_points = get_post_meta( $form_id, 'wppb_msf_break_points', true );
    } else {
        $break_points = get_option( 'wppb_msf_break_points', 'not_found' );
...
add_filter( 'wppb_form_submit_extra_attr', 'wppb_msf_hide_form_submit', 10, 3 );
 ...
 * @return  string
 */
function wppb_msf_after_form_fields( $output, $form_type, $form_id ) {
...
add_filter( 'wppb_after_form_fields', 'wppb_msf_after_form_fields', 10, 3 );
 ...
 * @return  string
 */
function wppb_msf_before_form_fields( $output, $form_type, $form_id ) {
    if( ! is_null( $form_id ) ) {
        $break_points = get_post_meta( $form_id, 'wppb_msf_break_points', true );
        $wppb_msf_options = get_post_meta( $form_id, 'wppb_msf_post_options', true );
        $wppb_msf_options = is_array( $wppb_msf_options ) ? $wppb_msf_options : 'not_found';
...
add_filter( 'wppb_before_form_fields', 'wppb_msf_before_form_fields', 10, 3 );
 ...
 * @return  string
 */
function wppb_msf_column_content( $content, $list, $meta ) {
...
add_filter( 'wck_add_content_after_columns', 'wppb_msf_column_content', 10, 3 );
 ...
 * @since   v.1.0.0
 */
function wppb_msf_save_break_points() {
    check_ajax_referer( 'wppb_msf_backend_nonce', 'wppb_msf_ajax_nonce' );

    if( $_POST['wppb_msf_form_id'] == 'manage-fields' ) {
        $break_points = get_option( 'wppb_msf_break_points', 'not_found' );
    } else {
        $break_points = get_post_meta( $_POST['wppb_msf_form_id'], 'wppb_msf_break_points', true );
        $break_points = is_array( $break_points ) ? $break_points : 'not_found';
    }

    if( $_POST['wppb_msf_action'] == 'add' ) {
        if( $break_points == 'not_found' ) {
            $break_points = array( intval( $_POST['wppb_msf_field_id'] ) => intval( $_POST['wppb_msf_field_id'] ) );
        } else {
            $break_points[intval( $_POST['wppb_msf_field_id'] )] = intval( $_POST['wppb_msf_field_id'] );
        }
    } elseif( $_POST['wppb_msf_action'] == 'remove' && $break_points != 'not_found' ) {
        unset( $break_points[intval( $_POST['wppb_msf_field_id'] )] );
    }

    if( $_POST['wppb_msf_form_id'] == 'manage-fields' ) {
        update_option( 'wppb_msf_break_points', $break_points );
    } else {
        update_post_meta( intval( $_POST['wppb_msf_form_id'] ), 'wppb_msf_break_points', $break_points );
    }

    die( 'option_updated' );
}
add_action( 'wp_ajax_wppb_msf_save_break_points', 'wppb_msf_save_break_points' );
 ...
 * @since   v.1.0.0
 */
function wppb_msf_delete_break_points( $post_meta, $post_id, $element_id = NULL ) {
    if( isset( $element_id ) ) {
...
add_action( 'wck_before_remove_meta', 'wppb_msf_delete_break_points', 10 , 3 );
add_action( 'wppb_before_remove_all_fields', 'wppb_msf_delete_break_points', 10 , 2 );
 ...
 * @since   v.1.0.0
 */
function wppb_msf_check_break_points() {
...
add_action( 'wp_ajax_wppb_msf_check_break_points', 'wppb_msf_check_break_points' );
add_action( 'wp_ajax_nopriv_wppb_msf_check_break_points', 'wppb_msf_check_break_points' );
 ...
 * @since   v.1.0.0
 */
function wppb_msf_after_reorder_check_break_points( $id ) {
    if( empty( $id ) ) {
        $break_points = get_option( 'wppb_msf_break_points', 'not_found' );
        $id = '\'manage-fields\'';
    } else {
    ...
    echo '<script>wppb_msf_break_points_buttons( '. $id .' )</script>';
}
add_action( 'wck_refresh_list_wppb_manage_fields', 'wppb_msf_after_reorder_check_break_points' );
add_action( 'wck_refresh_entry_wppb_manage_fields', 'wppb_msf_after_reorder_check_break_points' );
add_action( 'wck_refresh_list_wppb_rf_fields', 'wppb_msf_after_reorder_check_break_points' );
add_action( 'wck_refresh_entry_wppb_rf_fields', 'wppb_msf_after_reorder_check_break_points' );
add_action( 'wck_refresh_list_wppb_epf_fields', 'wppb_msf_after_reorder_check_break_points' );
add_action( 'wck_refresh_entry_wppb_epf_fields', 'wppb_msf_after_reorder_check_break_points' );
 ...
 * @since   v.1.0.0
 */
function wppb_msf_check_tab_titles() {
    if( $_POST['wppb_msf_form_id'] == 'manage-fields' ) {
        $tab_titles = get_option( 'wppb_msf_tab_titles', 'not_found' );
    } else {
        $tab_titles = get_post_meta( $_POST['wppb_msf_form_id'], 'wppb_msf_tab_titles', true );
...
add_action( 'wp_ajax_wppb_msf_check_tab_titles', 'wppb_msf_check_tab_titles' );
add_action( 'wp_ajax_nopriv_wppb_msf_check_tab_titles', 'wppb_msf_check_tab_titles' );
 ...
 * @since   v.1.0.0
 */
function wppb_msf_check_required_fields() {
    ...
    $request_data = array();
    foreach( $_POST['request_data'] as $key => $value ) {
        $key = sanitize_text_field( $key );
        $request_data[wppb_handle_meta_name( $key )] = $value;
    }

    foreach( $_POST['wppb_msf_fields'] as $id => $value ) {
        $field = array();
            ...
            if( function_exists( 'wppb_woo_billing_fields_array' ) && function_exists( 'wppb_check_woo_individual_fields_val' ) ) {
                ...
                if( isset( $value['fields'] ) ) {
                    foreach( $value['fields'] as $key => $woo_field_label ) {
                        ...
                        $woo_error_for_field = wppb_check_woo_individual_fields_val( '', $billing_fields[$key], $key, $request_data, $_POST['form_type'] );
            ...
            if( function_exists( 'wppb_woo_shipping_fields_array' ) && function_exists( 'wppb_check_woo_individual_fields_val' ) ) {
                ...
                if( isset( $value['fields'] ) ) {
                    foreach( $value['fields'] as $key => $woo_field_label ) {
                        ...
                        $woo_error_for_field = wppb_check_woo_individual_fields_val( '', $shipping_fields[$key], $key, $request_data, $_POST['form_type'] );
        ...
        if( isset( $value['extra_groups_count'] ) ) {
            $wppb_manage_fields = apply_filters( 'wppb_form_fields', $wppb_manage_fields, array( 'context' => 'multi_step_forms', 'extra_groups_count' => esc_attr( $value['extra_groups_count'] ), 'global_request' => $request_data, 'form_type' => $_POST['form_type'] ) );
        }

        // search for fields in fields array by meta-name or id (if field does not have a mata-name)
        if( ! empty( $value['meta-name'] ) && $value['meta-name'] != 'passw1' && $value['meta-name'] != 'passw2' && wppb_msf_get_field_options( $value['meta-name'], $wppb_manage_fields ) !== false ) {
            $field = wppb_msf_get_field_options( $value['meta-name'], $wppb_manage_fields );
        } elseif( ! empty( $field_id ) && wppb_msf_get_field_options( $field_id, $wppb_manage_fields, 'id' ) !== false
            && $field_name != 'woocommerce-customer-billing-address' && $field_name != 'woocommerce-customer-shipping-address' ) {

            $field = wppb_msf_get_field_options( $field_id, $wppb_manage_fields, 'id' );
        }
...
add_action( 'wp_ajax_wppb_msf_check_required_fields', 'wppb_msf_check_required_fields' );
add_action( 'wp_ajax_nopriv_wppb_msf_check_required_fields', 'wppb_msf_check_required_fields' );

/* Function that search in multidimensional arrays */
function wppb_msf_get_field_options( $needle, $haystack, $type = 'meta-name' ) {
...
*
*/
function wppb_msf_update_checker(){
    if( class_exists('wppb_PluginUpdateChecker') ) {
        ...
        $wppb_msf_update = new wppb_PluginUpdateChecker('http://updatemetadata.cozmoslabs.com/?localSerialNumber=' . $localSerial . '&uniqueproduct=CLPBMSF', __FILE__, 'wppb-multi-step-forms-add-on' );
    }
}
add_action( 'plugins_loaded', 'wppb_msf_update_checker', 100 );
